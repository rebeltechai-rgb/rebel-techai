...
def compute_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    plus_di = 100 * plus_dm.ewm(alpha=1 / period, adjust=False, min_periods=period).mean() / (smooth_tr + 1e-10)
    minus_di = 100 * minus_dm.ewm(alpha=1 / period, adjust=False, min_periods=period).mean() / (smooth_tr + 1e-10)
...
class RebelIntelligentScanner:
    ...
    def _analyze_trend_mtf(self, mtf_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        """Multi-timeframe trend analysis with ADX strength."""
        per_tf = {}
        votes = 0
        adx_value = 0.0

        # Compute ADX from highest available timeframe
        for tf_pref in ["H1", "H4", "M30", "M15", "M5"]:
            if tf_pref in mtf_data and len(mtf_data[tf_pref]) >= 30:
                adx_series = compute_adx(mtf_data[tf_pref])
                last_adx = float(adx_series.iloc[-1]) if not adx_series.empty else 0
        ...
        return {"per_tf": per_tf, "overall": overall, "trend_score": trend_score, "adx": round(adx_value, 2)}
